...
export default function SectionJobsModal({ section, jobs, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="bg-slate-card border border-white/10 rounded-2xl p-6 max-w-6xl w-full max-h-[85vh] overflow-hidden flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xl font-bold">{section.name}</h2>
            <p className="text-white/40 text-sm">{jobs.length} job{jobs.length !== 1 ? 's' : ''}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-auto">
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-slate-card">
              ...
              {sortedJobs.map((j) => {
                ...
                return (
                  <tr key={j.id} className="border-t border-white/5 hover:bg-white/5">
                    ...
                    {dateFieldLabel && (
                      <td className="py-3 px-3 text-white/80 tabular-nums text-xs">
                        {relevantDate || <span className="text-danger">missing</span>}
                      </td>
                    )}
                    <td className="py-3 px-3 text-white/60 tabular-nums text-xs">{j.promisedDate || '—'}</td>
