...
const STATUS_CONFIG = {
  ...
  no_date:  { label: 'No Date',  className: 'text-gray-500' },
};
...
function SubtaskRow({ name, assignee, due_on, completed, today }) {
  ...
  return (
    <div className="flex items-center gap-3 text-xs py-2 border-b border-gray-100 last:border-0">
      ...
      <span className={`flex-1 truncate ${completed ? 'text-gray-500 line-through' : 'text-gray-700'}`}>
        {name}
      </span>
      <span className="text-gray-500 text-[10px] shrink-0">{assignee ?? '—'}</span>
      {due_on ? (
        <span className={`text-[10px] tabular-nums shrink-0 ${isOverdue ? 'text-danger font-semibold' : 'text-gray-500'}`}>
          {due_on}
        </span>
      ) : (
        <span className="text-[10px] text-gray-500 shrink-0">no date</span>
      )}
    </div>
  );
}
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-lg bg-white border-l border-gray-200 z-50 overflow-y-auto shadow-2xl">
        <div className="p-6 space-y-6">
          ...
          <div className="flex flex-wrap gap-5">
            <div>
              <p className="text-gray-500 text-xs">Production Due</p>
            ...
            {job.projectedLate && (
              <div>
                <p className="text-gray-500 text-xs">Projection</p>
                <p className="text-sm font-semibold text-warning">⚠ Projected Late</p>
              </div>
            )}
            {job.redoType && (
              <div>
                <p className="text-gray-500 text-xs">Redo Type</p>
