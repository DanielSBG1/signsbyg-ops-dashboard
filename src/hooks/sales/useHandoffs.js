...
function cacheKey(period, start, end) { return `sbg_h_${period}_${start || ''}_${end || ''}`; }

export function useHandoffs(enabled = true) {
  ...
  const fetchHandoffs = useCallback(async () => {
    ...
    try {
      let url = `/api/v2/sales-handoffs?period=${period}`;
