...
function cacheKey(period, start, end) { return `sbg_p_${period}_${start || ''}_${end || ''}`; }
...
export function usePipeline(enabled = true) {
  ...
  return { data, loading, refreshing, error, period, setPeriod, customRange, setCustomRange, lastRefreshed, refresh: fetchPipeline };
}
